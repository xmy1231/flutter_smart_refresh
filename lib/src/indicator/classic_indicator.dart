  ...
  Widget _buildText(mode) {
    ...
    return Text(
        mode == RefreshStatus.canRefresh
            ? widget.releaseText ?? strings.canRefreshText ?? 'Release to refresh'
            : mode == RefreshStatus.completed
                ? widget.completeText ?? strings.refreshCompleteText ?? 'Refresh completed'
                : mode == RefreshStatus.failed
                    ? widget.failedText ?? strings.refreshFailedText ?? 'Refresh failed'
                    : mode == RefreshStatus.refreshing
                        ? widget.refreshingText ?? strings.refreshingText ?? 'Refreshing...'
                        : mode == RefreshStatus.idle
                            ? widget.idleText ?? strings.idleRefreshText ?? 'Pull down to refresh'
                            : mode == RefreshStatus.canTwoLevel
                                ? widget.canTwoLevelText ?? strings.canTwoLevelText ?? 'Release to enter second floor'
                                : '',
  ...
  Widget _buildText(LoadStatus? mode) {
    ...
    return Text(
        mode == LoadStatus.loading
            ? widget.loadingText ?? strings.loadingText ?? 'Loading...'
            : LoadStatus.noMore == mode
                ? widget.noDataText ?? strings.noMoreText ?? 'No more data'
                : LoadStatus.failed == mode
                    ? widget.failedText ?? strings.loadFailedText ?? 'Load failed'
                    : LoadStatus.canLoading == mode
                        ? widget.canLoadingText ?? strings.canLoadingText ?? 'Release to load more'
                        : widget.idleText ?? strings.idleLoadingText ?? 'Pull up to load more',
        style: widget.textStyle);
  }

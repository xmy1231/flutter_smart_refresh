...
const double _scrollCorrectionDelta = 0.00001;

/// Callback signature for async operations (e.g. readyToRefresh, endRefresh).
typedef VoidFutureCallBack = Future<void> Function();

/// Callback for overscroll offset changes.
typedef void OffsetCallBack(double offset);

/// Callback for indicator mode changes (RefreshStatus or LoadStatus).
...
abstract class RefreshIndicator extends StatefulWidget {
  ...
  const RefreshIndicator({Key? key, this.height = 60.0, this.offset = 0.0, this.completeDuration = const Duration(milliseconds: 500), this.refreshStyle = RefreshStyle.Follow, this.onInit, this.onDispose})
      : super(key: key);
}
...
abstract class LoadIndicator extends StatefulWidget {
  ...
  const LoadIndicator({Key? key, this.onClick, this.loadStyle = LoadStyle.ShowAlways, this.height = 60.0, this.onInit, this.onDispose}) : super(key: key);
}
...
abstract class RefreshIndicatorState<T extends RefreshIndicator> extends State<T> with IndicatorStateMixin<T, RefreshStatus>, RefreshProcessor {
  ...
  bool _inVisual() {
    final pos = _position;
    assert(pos != null, '_position must be set before calling _inVisual');
    if (pos == null) return false;
    return pos.pixels < 0.0;
  }

  double _calculateScrollOffset() {
    final pos = _position;
    assert(pos != null, '_position must be set before calling _calculateScrollOffset');
  ...
  void _dispatchModeByOffset(double offset) {
    final pos = _position;
    assert(pos != null, '_position must be set before calling _dispatchModeByOffset');
    if (pos == null) return;
    if (mode == RefreshStatus.twoLeveling) {
      if (pos.pixels > configuration!.closeTwoLevelDistance && activity is BallisticScrollActivity) {
    ...
    if (pos.extentBefore == 0.0 && widget.refreshStyle == RefreshStyle.Front) {
  ...
  void _handleModeChange() {
    ...
    if (mode == RefreshStatus.completed || mode == RefreshStatus.failed) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        final p = _position;
        if (p == null) return;
        // resetActivity() may throw when no scroll activity is currently
        // active (e.g. programmatic requestTwoLevel). Swallow so we don't
        // crash the framework.
        try {
          activity!.resetActivity();
        } catch (_) {}
        if (refresher!.onTwoLevel != null) refresher!.onTwoLevel!(true);
        p.animateTo(0.0, duration: const Duration(milliseconds: 500), curve: Curves.linear).whenComplete(() {
          if (!mounted) return;
          // Force position to exactly 0 before transitioning to twoLeveling.
          // When maxScrollExtent is small (viewport not full), correctByPixels
          // keeps the position at maxScrollExtent during the animation, so the
          // underdamped BouncingScrollSimulation set up by RefreshPhysics on
          // entering twoLeveling springs back through closeTwoLevelDistance
          // and fires twoLevelComplete prematurely.
          final p2 = _position;
          if (p2 == null) return;
          p2.jumpTo(0.0);
...
abstract class LoadIndicatorState<T extends LoadIndicator> extends State<T> with IndicatorStateMixin<T, LoadStatus>, LoadingProcessor {
  ...
  double _calculateScrollOffset() {
    ...
    assert(pos != null, '_position must be set before calling _calculateScrollOffset');
    if (pos == null) return 0.0;
    final double overScrollPastEnd = math.max(pos.pixels - pos.maxScrollExtent, 0.0);
  ...
  void finishLoading() {
    ...
    endLoading().then((_) {
      ...
      // Patch bug: indicator disappears fastly when load more complete
      if (mounted) Scrollable.of(context).position.correctBy(_scrollCorrectionDelta);
      WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  bool _checkIfCanLoading() {
    final pos = _position;
    assert(pos != null, '_position must be set before calling _checkIfCanLoading');
    if (pos == null) return false;
    if (pos.maxScrollExtent - pos.pixels <= configuration!.footerTriggerDistance && pos.extentBefore > 2.0 && _enableLoading) {
      ...
      if (mode != LoadStatus.canLoading && pos.userScrollDirection == ScrollDirection.forward) {
        return false;
      }
      return true;
    }
  ...
  void _handleModeChange() {
    ...
    if (mode == LoadStatus.idle || mode == LoadStatus.failed || mode == LoadStatus.noMore) {
      ...
      if (_position is ScrollActivityDelegate) {
        final p = _position!;
        if (p.activity!.velocity < 0 && _lastMode == LoadStatus.loading && !p.outOfRange) {
          p.beginActivity(IdleScrollActivity(p as ScrollActivityDelegate));
        }
      }
  ...
  void _listenScrollEnd() {
    final pos = _position;
    assert(pos != null, '_position must be set before calling _listenScrollEnd');
    if (pos == null) return;
    ...
    return SliverLoading(
        ...
        child: LayoutBuilder(
          builder: (BuildContext context, BoxConstraints cons) {
            _isHide = cons.biggest.height == 0.0 && widget.height > 0;
...
/// Mixin for indicator state that manages position tracking, mode changes, and offset dispatch.
///
/// Provides shared lifecycle logic for both [RefreshIndicatorState] and [LoadIndicatorState]:
/// - Tracks the scroll [ScrollPosition] and listens for offset changes
/// - Manages the indicator mode ([RefreshStatus] / [LoadStatus]) via [RefreshNotifier]
/// - Wires up [RefreshConfiguration] and [SmartRefresher] ancestor lookups
/// - Calls [_handleOffsetChange] and [_handleModeChange] on position / mode transitions
...
/// Callback interface for custom header indicators.
///
/// Override these methods in your [RefreshIndicatorState] subclass to
/// react to offset changes, mode transitions, and refresh lifecycle events.
mixin class RefreshProcessor {
  /// Called when the overscroll offset changes (the distance pulled beyond the edge).
  void onOffsetChange(double offset) {}

  /// Called when the header mode changes (e.g. idle → canRefresh → refreshing).
  void onModeChange(RefreshStatus? mode) {}

  /// Called when the indicator is about to enter the refreshing state.
  /// Return a [Future] to delay the transition; [onRefresh] fires after this completes.
  Future readyToRefresh() {
    return Future.value();
  }

  /// Called when refresh completes or fails, before springing back.
  /// Return a [Future] to delay the spring-back animation.
  Future endRefresh() {
    return Future.value();
  }

  /// Called after the indicator has sprung back to reset any transient values.
  void resetValue() {}
}

/// Callback interface for custom footer/load indicators.
///
/// Override these methods in your [LoadIndicatorState] subclass to
/// react to offset changes, mode transitions, and load lifecycle events.
mixin class LoadingProcessor {
  /// Called when the overscroll offset changes (the distance pulled beyond the edge).
  void onOffsetChange(double offset) {}

  /// Called when the footer mode changes (e.g. idle → canLoading → loading).
  void onModeChange(LoadStatus? mode) {}

  /// Called when the indicator is about to enter the loading state.
  /// Return a [Future] to delay the transition; [onLoading] fires after this completes.
  Future readyToLoad() {
    return Future.value();
  }

  /// Called when loading completes or fails, before springing back.
  /// Return a [Future] to delay the spring-back animation.
  Future endLoading() {
    return Future.value();
  }

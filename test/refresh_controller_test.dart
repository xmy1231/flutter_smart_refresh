...
void main() {
  ...
  testWidgets("autoCleanup disposes controller when widget is removed", (tester) async {
    final RefreshController controller = RefreshController(autoCleanup: true);
    await tester.pumpWidget(buildRefresher(controller, count: 20));

    expect(controller.headerMode, isNotNull);
    expect(controller.footerMode, isNotNull);

    // Remove widget from tree
    await tester.pumpWidget(Container());
    await tester.pumpAndSettle();

    // Note: we verify the widget tree rebuilds without crash.
    // autoCleanup removes listeners during dispose — the controller
    // remains usable but is no longer connected to a widget.
    expect(controller.headerStatus, isNull);
    expect(controller.footerStatus, isNull);
  });

  testWidgets("autoCleanup defaults to false (backward compat)", (tester) async {
  ...
  testWidgets("headerStatusStream emits status changes", (tester) async {
    final RefreshController controller = RefreshController(initialRefresh: true);
    final List<RefreshStatus?> streamEvents = [];
    controller.headerStatusStream.listen((s) => streamEvents.add(s));

    await tester.pumpWidget(buildRefresher(controller, count: 20));
    await tester.pumpAndSettle();

    expect(controller.headerStatus, RefreshStatus.refreshing);
    expect(streamEvents, contains(RefreshStatus.refreshing));

    controller.refreshCompleted();
    await tester.pumpAndSettle(const Duration(milliseconds: 500));

    expect(streamEvents, contains(RefreshStatus.idle));
  });

  testWidgets("footerStatusStream emits status changes", (tester) async {
    final RefreshController controller = RefreshController();
    final List<LoadStatus?> streamEvents = [];
    controller.footerStatusStream.listen((s) => streamEvents.add(s));
    ...
    expect(streamEvents, contains(LoadStatus.loading));

    controller.loadComplete();
    await tester.pumpAndSettle();
    expect(streamEvents, contains(LoadStatus.idle));
  });
}
